...
test('basic application functionality', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /Mind Map PWA/i, level: 1 })).toBeVisible();
  
  // Check that the tabs are visible
  await expect(page.getByRole('tab', { name: /Mind Map/i })).toBeVisible();
  ...
  // Click back to the Mind Map tab
  await page.getByRole('tab', { name: /Mind Map/i }).click();
  
  // Check that the Mind Map content is visible
  await expect(page.getByText(/Mind Map/i)).toBeVisible();
});
