...
const ReactFlowAdapter: React.FC<ReactFlowAdapterProps> = ({
  ...
  const handleConnect = useCallback(
    (connection: Connection) => {
      ...
      // Update local state
      setEdges(prevEdges => [...prevEdges, addedEdge as unknown as Edge]);
    },
  ...
  const handleNodeClick = useCallback(
    (_: React.MouseEvent, node: Node) => {
      if (onNodeClick) {
        // Cast to VersionedNode with type assertion
        const versionedNode = node as unknown as VersionedNode;
        onNodeClick(versionedNode);
      }
    },
    [onNodeClick]
  );
  ...
  const handleEdgeClick = useCallback(
    (_: React.MouseEvent, edge: Edge) => {
      if (onEdgeClick) {
        // Cast to VersionedEdge with type assertion
        const versionedEdge = edge as unknown as VersionedEdge;
        onEdgeClick(versionedEdge);
      }
    },
    [onEdgeClick]
  );
